...
type Shell struct {
	...
	/* NOTE: must use revoke() to close the channel. */
	currResp chan string
	currCmd  chan *ShellSyncObj

	tmpDanglingBuf string // tmpDanglingBuf stores the typed text
	quoted         string // quoted handles for the unclosed string sign.
	DefaultPrompt  string
	PendingPrompt  string
	needCmdChan    bool
	// shouldContinued is an internal variable for maintaining prompt when there are multiple lines
...
func (s *Shell) FlipPromptCheck(currLine string) bool {
	...
	if isUnclosed {
		s.quoted = quoteType
		s.setPrompt("quote "+quoteType+s.PendingPrompt, ColorWarn)
	...
	if s.shouldContinued { // only odd backslash
		s.tmpDanglingBuf += currLine
		if len(s.quoted) == 0 {
			s.setPrompt(s.PendingPrompt, ColorWarn)
		}
		return true
	} // else {
...
func (s *Shell) SetCurrResp(resp *ShellSyncObj) {
	if !s.needCmdChan || resp == nil || s.cannotSend.Load() {
		return
	}
	s.wg.Go(func() {
		select {
...
func NewShell(
	...
	res := &Shell{
		editor:          NewLineEditor(r, w, rules.LFisCRLF, rules.FullCRLF),
		tmpDanglingBuf:  "",
		shouldContinued: false,
		currResp:        make(chan string),
		currCmd:         make(chan *ShellSyncObj),
		needCmdChan:     rules.NeedHijackCmd,
	}
	res.DefaultPrompt = rules.DefaultPrompts
	res.PendingPrompt = rules.PendingPrompts
	res.setPrompt(rules.DefaultPrompts, ColorReset)
	return res
}
