...
func fieldAttrsHandler(
	...
	fieldAttrs := v["attr"].(map[string]any)
	var (
		bsonTagFlag = false
		jsonTagFlag = false
		needCounter = false
	)
	var snakeFieldName = LowerSnakeConvertor(fieldName)
	// handle attributes
	for fieldAttr, maybeStore := range fieldAttrs {
		if fieldAttr == "foreign" {
			...
			currField[fieldName].Bson += LowerSnakeConvertor(fieldName) + ",omitempty;"
			bsonTagFlag, jsonTagFlag = true, true

		} else if isAttrForEasiestStructTag(fieldAttr) {
			// primary||notnull||autoincrement||unique
			currField[fieldName].Gtag += attrConvertor(fieldAttr)
			...
			var counterStr = LowerSnakeConvertor("CounterFor" + fieldName)
			var extFieldInfo = fieldRec{
			...
			if currField[fieldName].Type == "int64" {
				_, err := strconv.ParseInt(maybeStore.(string), 2, 64)
				if err != nil {
					panic(err.Error())
				}
			}
			currField[fieldName].Gtag += fmt.Sprintf(
	...
	currField[fieldName].Gtag += fmt.Sprintf(
		"column:%s;", LowerSnakeConvertor(fieldName),
	)
	if !bsonTagFlag {
		currField[fieldName].Bson += fmt.Sprintf(
			"%s,omitempty", snakeFieldName,
		)
	}
	if !jsonTagFlag {
		currField[fieldName].Json += fmt.Sprintf(
			"%s,omitempty", snakeFieldName,
		)
	}
	(*res)[tabName] = append((*res)[tabName], currField)
	if needCounter {
		(*auxFunc).Add(tabName)
	}
}
...
func RDTGenAux(
	...
	for tabName, tabMemArr := range listener.MetaStruct {
		...
		for fieldName, v := range tabMemArr {
			...
			fieldAttrsHandler(
				listener, tabName, fieldName,
				v.(map[string]any), &res, &auxFunc,
			)
		}
	}
