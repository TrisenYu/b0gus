...
Additionally, the configuration is read from `configs/config.toml` by default. 
You must refer to the content defined in `configs/example.toml` 
to correctly deploy the service configurations you want to enable.
Similarly, [Python script](../../auto_translator.py) is provided in the project root directory 
for invoking the LLM to translate locale files.
When you intend to modify this Python script, 
you also need to follow the definitions specified in `example-translator-conf.toml` for configuration.
...
3. To meet internationalization requirements, all comments in business code must be written in English.
...
def foo_snake(
    ...
    # The closing parenthesis, return type, and opening brace (if applicable) of the function 
...
/* the definition form of function `do_something` is also acceptable. */ 
...
### References
mentioned in code or documentations.

### License
b0gus is distributed under the terms of the BSD 3-Clause License.
See the included file `license` in the root directory of b0gus for more details.

### Todo-list
- if possible, set up [oss-fuzz](https://google.github.io/oss-fuzz/getting-started/new-project-guide/)
- solve other todos mentioned in the source code
