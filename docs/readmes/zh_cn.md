...
后者如果处在Linux环境Debian系发行版下，可直接通过`sudo apt install -y protoc-gen-go`而获取。
...
类似地，工程主目录下提供有调用LLM翻译locale文件的[python脚本](../../tools/auto_translator.py)。
当有意修改并尝试运行该脚本时，也需要参考`example-translator-conf.toml`内的定义来提供其配置文件`translator-conf.toml`。

暂不支持本地安装与配置为用户级服务。

### 贡献规范
代码合并请求需满足以下全部要求，方可合并：
1. Fork这个仓库。
2. 基于dev分支新建`feat-xxx`分支，即：`git checkout -b feat-xxx`。此处`xxx`指代你为某个问题或者缺乏的实现所要新加的功能。
3. 提交前，请遵循[代码质量规范](#代码质量规范)。 对于提交命名，
   请使用feat, chore, add, del等领域内可读的缩略注明功能性新增或裁剪语句，并附着最主要修改的代码文件和变更内容上的说明。
   同时，记得将你的信息填入[作者列表](../AUTHORS)中.
4. 当你认为你的实现没有逻辑错误并且在事实上能让代码在你的本地环境中正常运行
  （即功能不产生逻辑错误、并发风险、内存溢出或越界读写等异常行为），
   ...
   同时，在发起合并请求之前，**请务必确保请求内所有提交都是受验证的(verified)**。
5. 所有合并请求，必须经至少一位非本次提交的项目维护者审核通过，且需符合[合并请求建议](#合并请求建议)的相关要求。

经代码审阅与额外综合评测后，你的分支将会被合并，你也将成为b0gus的贡献者。

#### 合并请求建议
参与贡献可先从简易待处理问题入手进行修复。

相较于反复将主分支/主要开发分支代码并入功能分支，优先对合并请求执行变基操作，从而避免提交历史的杂乱。
当评审人提出修改意见，请直接使用`git commit --amend`修订原有提交记录。
在此基础上需同步你fork出的分支时，请使用`git push --force-with-lease`强制推送更新。

#### 代码质量规范
本仓库内对所编写代码存在限制性要求：
1. 单个函数内，**缩进在原则上不应超过 $4$ 层**，且行数不应超过 $100$ 行，每列字符不应超过 $110$ 个。
   1. 如果不可避免超过 $4$ 层而小于等于 $5$ 层，则该层内连同注释的行数，不应连续超过 $10$ 行。
   2. 拒绝评阅与通过达到或超过 $6$ 层嵌套的实现逻辑。
   3. golang的结构体标签字段在列数上可不满足这一要求。
